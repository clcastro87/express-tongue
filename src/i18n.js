...
exports.localize = function (options) {
    // Set default options if not present
    options = options || {};
    const defaultLangAbbr = options.defaultLangAbbr || "es";
    const languagesPath = options.path || "src/server/i18n";
    ...
    function initialize() {
        ...
        const configLanguages = config.get("languages") || [];
    ...
    function buildLang(lang) {
        ...
        let keyValues = _.extend({}, require(langPath));
    ...
    function getLangs(langString) {
        // Parse languages
        const langArray = parseLangs(langString);
